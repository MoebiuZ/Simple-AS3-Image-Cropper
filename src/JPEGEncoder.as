package {
	...
	private function writeAPP1(texto:String):void {
			
			var tamano;
			var tamText;
			
			if (texto.length%2) {
				tamano = 34 + texto.length + 1;
				tamText = texto.length + 1;
			} else {
				tamano = 34 + texto.length;
				tamText = texto.length;				
			}
			
			writeWord(0xFFE1); // marker
			writeWord(tamano); // length
			writeByte(0x45); // E
			writeByte(0x78); // x
			writeByte(0x69); // i
			writeByte(0x66); // f
			...
			writeWord(0x4D4D); // byte order ("II")
			writeWord(0x002A); // 42
			writeWord(0);			
			writeWord(0x0008); // 0th IFD offset
			writeWord(0x0001); // Number of Interoperability
			writeWord(0x010E); // Image description TAG (Ahora CopyRIght)
			writeWord(0x0002); // Type
			writeWord(0);
			writeWord(tamText); // Count

			
			writeWord(0); // valueOffset
			writeWord(26);							


			
			writeWord(0);  // NEXT IFD OFFSET
			writeWord(0);
			
			for (var i = 0; i < texto.length; i++) {
				writeByte(texto.charCodeAt(i));
		...
		public function encode(image:BitmapData, nombre:String):ByteArray
		{
